...
//     outputRequirements: [
//       { key: "voiceMatch", label: "Voice Match", icon: UserCheck, type: "addition", amount: 0 },
//       { key: "aiVoiceover", label: "AI Voiceover", icon: Bot, type: "addition", amount: 0 },
...
export const pricingData = {
  languages: [
      "English", "Hindi", "Tamil", "Arabic", "Spanish", "French", "Russian", "German", "Japanese",
  ],
  dialects: {
      "English": ["British", "American", "Australian", "Canadian"],
      "Hindi": ["Standard", "Bhojpuri", "Awadhi"],
      "Tamil": [], "Arabic": [], "Spanish": ["Castilian", "Mexican", "Argentine"],
      "French": ["Standard", "Canadian", "Belgian"], "Russian": [], "German": ["Standard", "Swiss", "Austrian"],
      "Japanese": ["Standard", "Kansai", "Tohoku"]
  },
  pricingRates: [
      { Source: "English", Target: "Hindi", Animation: [375, 375, 375, 375, 375, 350, 350, 350], Live: [400, 350, 350, 350, 300, 275, 275, 275] },
      { Source: "English", Target: "Tamil", Animation: [475, 475, 475, 475, 475, 450, 450, 450], Live: [500, 450, 450, 450, 350, 325, 325, 325] },
      { Source: "English", Target: "Arabic", Animation: [600, 600, 600, 600, 600, 575, 575, 575], Live: [600, 580, 580, 580, 525, 475, 475, 475] },
      { Source: "English", Target: "Spanish", Animation: [600, 600, 600, 600, 600, 575, 575, 575], Live: [600, 580, 580, 580, 525, 475, 475, 475] },
      { Source: "English", Target: "French", Animation: [600, 600, 600, 600, 600, 575, 575, 575], Live: [600, 580, 580, 580, 525, 475, 475, 475] },
  ],
  availableFiles: [
      { key: "premixFiles", label: "Premix File", icon: Download, type: "deduction", amount: -50 },
      { key: "meFiles", label: "M&E File", icon: Download, type: "deduction", amount: -50 },
...
const getLanguagePairRate = (source, target, contentType) => {
  const rateObj = pricingData.pricingRates.find((obj) => obj.Source === source && obj.Target === target);
  if (!rateObj) {
      console.warn("No pricing data found for", source, "to", target);
      return 0;
  }
...
// export const calculatePrice = (state) => {
//   const pairRate = getLanguagePairRate(state.selectedLanguage, state.selectedTargetLanguage, state.contentType);
//   const minutes = state.duration.hours * 60 + state.duration.minutes;
//   let basePrice = pairRate * minutes;
...
//   const effectiveBase = Math.max(0, basePrice + deductions);
//   const total = effectiveBase + additions;

//   return {
//       base: effectiveBase.toFixed(2),
//       additional: additions.toFixed(2),
//       total: total.toFixed(2),
//   };
// };
export const calculatePrice = (state) => {
    // Get base rate for language pair
    const pairRate = getLanguagePairRate(
    ...
    // Calculate deductions from available files
    let deductions = 0;
    Object.entries(state.availableFiles).forEach(([key, isSelected]) => {
      if (isSelected) {
        const fileOption = pricingData.availableFiles.find(opt => opt.key === key);
        if (fileOption?.type === "deduction") {
          deductions += fileOption.amount;
        }
      }
    });
    
    // Calculate additions from requirements
    let additions = 0;
    Object.entries(state.requirements).forEach(([key, isSelected]) => {
      if (isSelected) {
        const reqOption = pricingData.outputRequirements.find(opt => opt.key === key);
        if (reqOption?.type === "addition") {
          if (key === "lipMatch") {
            additions += reqOption.amount * minutes;
          } else if (key === "soundBalancing") {
            additions += reqOption.amount;
